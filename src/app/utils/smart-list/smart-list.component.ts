...
export class SmartListComponent implements OnInit, DoCheck, OnChanges {
  @Input() model: any;
  ...
  ngDoCheck() {
    ...
    this.list.forEach((elt, i) => {
      ...
      if (objChanges) {
        ...
        if (elt && (Object.keys(elt).length === Object.keys(this.defaultItem).length) && equals(elt, this.defaultItem)
          && this.model.length && this.model.indexOf(elt) > -1 && !elt[this.autoDeleteException]) {
